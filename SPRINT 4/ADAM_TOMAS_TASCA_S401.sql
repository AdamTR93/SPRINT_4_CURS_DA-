...
SELECT ROUND(AVG(t.amount), 2) as MediaAmount,iban
FROM transactions t
JOIN companies c
...
-- realizar un row_number para que muestre solo 2 decimales
...
SELECT COUNT(card_id) AS Num_TarjetasActivas, card_status
...
CREATE TABLE IF NOT EXISTS products_transactions AS 
